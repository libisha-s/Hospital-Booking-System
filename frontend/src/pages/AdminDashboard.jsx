...
function AdminDashboard() {
  ...
  const statCards = [
    ...
    },
    {
      ...
      icon: <FaRegCalendarCheck className="text-emerald-500 w-7 h-7" />,
  ...
  return (
    <div className="space-y-8">
      ...
      <div>
        ...
        <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">Here is a quick snapshot of the HealthCare+ operational status.</p>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        {statCards.map((card, idx) => (
          <Link
            key={idx}
            to={card.link}
            className="bg-white dark:bg-slate-800 rounded-3xl p-6 shadow-premium hover:shadow-premium-hover transition-all duration-300 border border-slate-100 dark:border-slate-700/60 flex items-center justify-between group cursor-pointer"
          >
            <div className="space-y-1">
              <span className="text-xs font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wider">{card.title}</span>
              <h2 className="text-3xl font-extrabold text-slate-850 dark:text-white group-hover:text-sky-500 transition-colors">
                {card.value}
              </h2>
            </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-7 bg-white dark:bg-slate-800 p-6 rounded-3xl shadow-premium border border-slate-100 dark:border-slate-700/60 space-y-4">
          <div className="flex items-center justify-between border-b border-slate-100 dark:border-slate-700 pb-3">
            <h3 className="text-lg font-bold text-slate-850 dark:text-white">Recent Confirmed Bookings</h3>
          ...
          <div className="space-y-4">
            {appointments.slice(-3).reverse().map((app) => {
              ...
              return (
                <div key={app.id} className="flex items-center justify-between p-3 rounded-2xl bg-slate-50 dark:bg-slate-900 border border-slate-100 dark:border-slate-800/80">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h4 className="text-sm font-bold text-slate-800 dark:text-white">{app.patientName}</h4>
                      <p className="text-xs text-slate-400">Consulting {doc.name}</p>
                    </div>
                  </div>

                  <div className="text-right space-y-1">
                    <span className="inline-block px-2.5 py-0.5 text-[10px] font-bold rounded-full bg-emerald-50 text-emerald-600 dark:bg-emerald-950/20 dark:text-emerald-400">
                      {app.status}
                    </span>
        ...
        <div className="lg:col-span-5 bg-white dark:bg-slate-800 p-6 rounded-3xl shadow-premium border border-slate-100 dark:border-slate-700/60 space-y-4">
          <div className="flex items-center justify-between border-b border-slate-100 dark:border-slate-700 pb-3">
            <h3 className="text-lg font-bold text-slate-850 dark:text-white">Active Roster Summary</h3>
            ...
            {doctors.slice(0, 3).map((doc) => (
              <div key={doc.id} className="flex items-center gap-3">
                ...
                <div className="text-right">
                  <span className="text-sm font-bold text-slate-700 dark:text-slate-350">{doc.experience}</span>
